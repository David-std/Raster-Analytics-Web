...
def test_temporal_split_is_ordered_and_2025_is_not_benchmark_data() -> None:
    split = TemporalSplit.current_fatal_outcome_split()
    ...
    with pytest.raises(ValueError, match="ordered and non-overlapping"):
        TemporalSplit(
            train=DateWindow("train", "2021-01", "2023-03"),
...
def test_builder_preserves_missingness_instead_of_imputing_zero() -> None:
    catalog = load_contract_catalog(CATALOG_PATH)
    builder = LeakageSafeDatasetBuilder(catalog, TemporalSplit.current_fatal_outcome_split())

    rows = builder.build(
        target_id="pedestrian_linked_fatal_count",
        feature_ids=["month_of_year_sin", "transit_stop_density"],
        targets=[TargetObservation("pedestrian_linked_fatal_count", "cell-1", "2024-01", 1)],
        features=[
            FeatureObservation(
            ...
            FeatureObservation(
                ...
                spatial_unit_id="cell-1",
                target_period_id="2024-01",
                state=ObservationState.OUTSIDE_SOURCE_COVERAGE,
                value=None,
            ),
        ],
    )
    ...
    assert (
        ...
        is ObservationState.OUTSIDE_SOURCE_COVERAGE
    )
...
def test_builder_inserts_missing_state_when_feature_observation_is_absent() -> None:
    ...
    row = builder.build(
        ...
        targets=[TargetObservation("pedestrian_linked_fatal_count", "cell-1", "2022-04", 0)],
        features=[],
    )[0]

    assert row.feature_values["month_of_year_cos"] is None
    assert row.feature_states["month_of_year_cos"] is ObservationState.MISSING
...
def test_non_observed_missingness_states_cannot_carry_fabricated_values() -> None:
    ...
    with pytest.raises(ValueError, match="must not fabricate a value"):
        builder.build(
            ...
            targets=[TargetObservation("pedestrian_linked_fatal_count", "cell-1", "2024-01", 0)],
            features=[
                FeatureObservation(
                    feature_id="transit_stop_density",
                    spatial_unit_id="cell-1",
